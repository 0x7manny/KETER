...
import {HonkVerifier as UltraVerifier} from "./UltraVerifier.sol";
...
contract KeterToken {
     ...
     * @param _publicInputs   Public inputs matching Noir circuit:
     *                         [0] merkle_root
     *                         [1] nullifier
     *                         [2] recipient
     *                         [3] sender
    ...
    function transferWithProof(
        ...
        _verifyTransferProof(_proof, _publicInputs, msg.sender, _to, _value);
        _transferFromDefaultPartitions(msg.sender, _to, _value, "");
     ...
     * @param _publicInputs   Public inputs (same layout as transferWithProof)
     */
     ...
     * @dev Sender proof: proves sender is in Merkle tree + authorized to send _value to _to.
     *      Receiver proof: proves receiver is in Merkle tree (sender field = _to's wallet).
     */
    function transferWithDualProof(
        ...
        // Sender proves: "I am msg.sender, I'm sending _value to _to"
        _verifyTransferProof(_senderProof, _senderPublicInputs, msg.sender, _to, _value);
        // Receiver proves: "I am _to, I'm in the Merkle tree" (membership only)
        _verifyMembershipProof(_receiverProof, _receiverPublicInputs, _to);
        _transferFromDefaultPartitions(msg.sender, _to, _value, "");
    ...
    /**
     * @dev Verify a full transfer ZK proof against the Noir circuit.
     *      Circuit public inputs layout:
     *        [0] merkle_root       — must match Registry
     *        [1] nullifier         — must be unused (anti-replay)
     *        [2] recipient         — must match _to
     *        [3] sender            — must match _from (= msg.sender)
     *        [4] transfer_amount   — must match _value
     */
    function _verifyTransferProof(
        ...
        require(_publicInputs.length == 5, "KT: need 5 public inputs");

        // 1. Merkle root matches Registry
        require(
            _publicInputs[0] == registry.getMerkleRoot(),
            "KT: root mismatch"
        );

        // 2. Nullifier not already used (anti-replay)
        bytes32 nullifier = _publicInputs[1];
        require(!usedNullifiers[nullifier], "KT: proof already used");
        usedNullifiers[nullifier] = true;

        // 3. Recipient in proof matches actual recipient
        require(
            _publicInputs[2] == bytes32(uint256(uint160(_to))),
            "KT: recipient mismatch"
        );

        // 4. Sender in proof matches msg.sender (prevents front-running)
        require(
            _publicInputs[3] == bytes32(uint256(uint160(_from))),
            "KT: sender mismatch"
        );

        // 5. Amount in proof matches actual transfer amount
        require(
            _publicInputs[4] == bytes32(_value),
            "KT: amount mismatch"
        );

        // 6. Verify the ZK proof via UltraVerifier
        require(
            verifier.verify(_proof, _publicInputs),
            "KT: invalid proof"
        );

        emit ProofVerified(msg.sender, _publicInputs[0], nullifier, block.timestamp);
    }

    /**
     * @dev Verify a membership-only ZK proof (for receiver in dual-proof transfers).
     *      Only checks: root, nullifier, and sender == _who (proves _who is in the tree).
     *      Accepts 5 public inputs but only enforces root, nullifier, and sender binding.
     */
    function _verifyMembershipProof(
        ...
        // 2. Nullifier not already used
        bytes32 nullifier = _publicInputs[1];
        require(!usedNullifiers[nullifier], "KT: proof already used");
        usedNullifiers[nullifier] = true;

        // 3. Sender field proves _who is in the tree
        require(
            _publicInputs[3] == bytes32(uint256(uint160(_who))),
            "KT: membership sender mismatch"
        );

        // 4. Verify the ZK proof via UltraVerifier
        require(
            verifier.verify(_proof, _publicInputs),
            "KT: invalid proof"
        );

        emit ProofVerified(_who, _publicInputs[0], nullifier, block.timestamp);
    }
